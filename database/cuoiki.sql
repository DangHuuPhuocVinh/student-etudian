...
from GIAOVIEN g,BOMON b,THAMGIADT t,DETAI d, 
where g.MABM = b.MABM and b.TENBM = N'Hệ Thống Thông Tin' and g.MAGV= t.MAGV and
 t.MADT=d.MADT 
group by d.MADT, d.TENDT
having count(distinct g.MAGV) >=all(


select count(distinct g.MAGV)
from GIAOVIEN g1,BOMON b1, THAMGIADT t1,DETAI d1
where g1.MABM = b1.MABM and b1.TENBM = N'Hệ Thông Thông Tin' and
g1.MAGV = t1.MAGV and t1.MADT=d1.MADT 
group by g1.magv)

--câu 2
select g.HOTEN, count (DISTINCT tg.MADT) as ThamGiaDeTai
from GIAOVIEN g, BOMON b, THAMGIADT t
where g.MABM = b.MABM and TENBM like N'Hệ thống thông tin'
  and g.MAGV = t.MAGV
group by g.MAGV, g.HOTEN
having count (DISTINCT t.MADT) = (select count (DISTINCT DETAI.MADT)
								   from DETAI)
